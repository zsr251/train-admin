<template>
	<el-card class="mod__menu">
		...
		>
			<el-table-column prop="name" label="名称" header-align="center" min-width="150">
				<template #default="scope">
					{{ scope.row.name }} <train-icon v-if="scope.row.hasDataPermission === 1" icon="icon-align-left" color = "red"></train-icon>
				</template>
			</el-table-column>
			<el-table-column prop="icon" label="图标" header-align="center" align="center">
				<template #default="scope">
					<train-icon :icon="scope.row.icon"></train-icon>
				</template>
			</el-table-column>
			...
			<el-table-column label="操作" fixed="right" header-align="center" align="center" width="230">
				<template #default="scope">
					<el-button v-if="scope.row.type === 0" v-auth="'sys:menu:save'" type="primary" link @click="addOrUpdateHandle(false, scope.row)"
						>新增</el-button
					>
					<el-button v-if="scope.row.url" type="primary" link @click="handleOpenDataPermission(scope.row)">数据权限</el-button>
		...
		<!-- 数据权限 -->
		<el-drawer v-if="dataPermissionVisible" v-model="dataPermissionVisible" :title="dataPermissionTitle" :size="1000" :close-on-press-escape="false">
			<data-permission ref="dataPermissionRef" :component="curComponent"></data-permission>
		</el-drawer>
	</el-card>
</template>
...
const dataPermissionTitle = ref('')
const dataPermissionRef = ref()
const curComponent = ref('')

const handleOpenDataPermission = (row: any) => {
	console.log(row)
	dataPermissionTitle.value = '数据权限 - ' + row.name
	dataPermissionVisible.value = true
	curComponent.value = row.url
}
