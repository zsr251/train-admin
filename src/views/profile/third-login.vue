	...
	<el-table :data="thirdList">
		...
		<el-table-column prop="openType" label="平台类型" align="center">
			<template #default="scope">
				<el-text v-if="scope.row.openType === 'wechat_work'"><train-icon icon="icon-workweixin" size="16" /> 企业微信</el-text>
				<el-text v-else-if="scope.row.openType === 'dingtalk'"><train-icon icon="icon-dingding" size="16" /> 钉钉</el-text>
				<el-text v-else-if="scope.row.openType === 'feishu'"><train-icon icon="icon-feishu" size="16" /> 飞书</el-text>
				<el-text v-else-if="scope.row.openType === 'wechat_open'"><train-icon icon="icon-weixin" size="16" /> 微信</el-text>
			</template>
		</el-table-column>
